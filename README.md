...
- 项目专属反 AI 味配置：`references/project-anti-ai-profile.md` 提供可选的项目级词表、病灶、动作库存、母题句式、场景卡和评分模板，用于区分必要类型质感与真实模板化风险。
...
当某个项目反复出现更细的 AI 味问题时，它还可以建立项目专属反 AI 味配置文件，例如 `10-anti-ai-profile.md`、`11-anti-ai-profile.md` 或 `anti-ai-profile.md`。这个文件不是通用禁词表，而是记录：

- 哪些类型词、设定词、职业词是本书允许保留的
- 哪些词句、动作、标签、标点习惯在本书里已经变成高危模板
- 哪些母题或回调句式需要变化，不能每次按同一语法按钮重复
- 哪些场景类型需要单独检查“过度防 AI 味”导致的失真
- 单段、单场景或整章的 `0-3` 评分与修复优先级

这部分是从真实项目经验中抽象出来的通用方法，不包含任何具体项目的私有设定或情节外壳。
...
| 项目反 AI 层 | 可选项目词表、病灶库、动作库存、母题句式、场景卡和评分线 |
